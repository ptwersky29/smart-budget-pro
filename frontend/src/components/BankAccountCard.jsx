    ...
    return (
      ...
      >
        <div className="relative shrink-0 h-8 w-8 rounded-lg bg-white dark:bg-secondary/40 border border-border/30 flex items-center justify-center overflow-hidden">
          {logoUrl ? (
            <img src={logoUrl} alt={institution} className="h-6 w-6 object-contain group-hover:scale-110 transition-transform duration-200" loading="lazy" onError={(e) => { e.target.onerror = null; e.target.src = `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><rect width="32" height="32" rx="8" fill="${bankColor}"/><text x="16" y="22" text-anchor="middle" font-family="system-ui,sans-serif" font-weight="700" font-size="18" fill="white">${(name || "?")[0].toUpperCase()}</text></svg>`)}`; }} />
          ) : (
            <Building2 className="h-3 w-3 text-muted-foreground" />
          )}
        </div>
        <div>
          <p className="text-xs font-medium leading-tight group-hover:text-emerald transition-colors">{name}</p>
        ...
        <div className="p-4">
          <div className="flex items-start gap-3">
            <div className="relative shrink-0 h-11 w-11 rounded-xl bg-white dark:bg-secondary/40 border border-border/30 flex items-center justify-center overflow-hidden shadow-sm">
              {logoUrl ? (
                <img src={logoUrl} alt={institution} className="h-8 w-8 object-contain group-hover:scale-110 transition-transform duration-200" loading="lazy" onError={(e) => { e.target.onerror = null; e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }} />
              ) : null}
              {!logoUrl && <Building2 className="h-5 w-5 text-muted-foreground" />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className="font-semibold text-sm truncate group-hover:text-emerald transition-colors">{name}</p>
                {type && (
              ...
              <p className="text-xs text-muted-foreground mt-0.5 truncate">{institution || type || ""}</p>
            </div>
            <div className="text-right shrink-0">
              <p className="text-base font-semibold tracking-tight">£{balanceFmt}</p>
  ...
  return (
    ...
    >
      <div className="relative shrink-0 h-9 w-9 rounded-lg bg-white dark:bg-secondary/40 border border-border/30 flex items-center justify-center overflow-hidden">
        {logoUrl ? (
          <img src={logoUrl} alt={institution} className="h-7 w-7 object-contain group-hover:scale-110 transition-transform duration-200" loading="lazy" onError={(e) => { e.target.onerror = null; e.target.src = `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><rect width="32" height="32" rx="8" fill="${bankColor}"/><text x="16" y="22" text-anchor="middle" font-family="system-ui,sans-serif" font-weight="700" font-size="18" fill="white">${(name || "?")[0].toUpperCase()}</text></svg>`)}`; }} />
        ) : (
          <Building2 className="h-4 w-4 text-muted-foreground" />
        )}
      </div>
      <div>
        <p className="text-sm font-medium leading-tight group-hover:text-emerald transition-colors">{name}</p>
