...
export default React.memo(function CashFlowChart({ overview, visibility = true, chartStyle = "smooth" }) {
  ...
  const lineType = chartStyle === "sharp" ? "linear" : "monotone";
  return (
        ...
        {overview?.monthly_flow?.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={overview.monthly_flow}>
              ...
              <Line type={lineType} dataKey="income" stroke="hsl(var(--emerald))" strokeWidth={2} dot={false} />
              <Line type={lineType} dataKey="spend" stroke="hsl(var(--topaz))" strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        ) : (
