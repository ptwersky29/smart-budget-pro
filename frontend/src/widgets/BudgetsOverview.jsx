...
export default React.memo(function BudgetsOverview({
  budgets,
  currentMonthBudget,
}) {
  return (
    <div className="rounded-2xl border border-border bg-card/90 backdrop-blur-xl shadow-card p-5">
      ...
      {!budgets || budgets.length === 0 ? (
        <Link
          to="/budgets"
          className="flex items-center gap-3 p-3 rounded-xl border border-dashed border-border text-sm text-muted-foreground hover:text-foreground hover:border-emerald/40 transition-colors"
        >
            ...
            return (
              <div key={b.budget_id}>
                <div className="mb-1 flex items-center justify-between gap-3 text-xs">
                  ...
                  />
                  <span
                    className={`tabular-nums ${over ? "text-ruby font-medium" : "text-muted-foreground"}`}
                  >
                ...
                <div className="h-2 bg-secondary rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${over ? "bg-ruby" : "bg-gradient-to-r from-emerald to-emerald/70"}`}
                    style={{ width: `${pct}%` }}
                  />
                </div>
              </div>
