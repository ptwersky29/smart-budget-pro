...
export default function AccountPage() {
  ...
  const defaultFilters = useMemo(() => ({ search: "", category: "", source: "", tx_type: "", date_from: "", date_to: "", amount_min: "", amount_max: "", sort: "date", order: "desc" }), []);
  ...
  const buildParams = useCallback(() => {
    const params = { connection_id: connectionId, limit, sort: filters.sort, order: filters.order, offset };
  ...
  return (
    <div className="space-y-6" data-testid="account-page">
      ...
      <SectionCard eyebrow="Transactions" title={`${totalTx} transaction${totalTx !== 1 ? "s" : ""} from this account`}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-2 pb-4">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
            ...
            <span className="text-muted-foreground/30">·</span>
            <span>Net <strong className={`font-medium tabular-nums ${netTotal >= 0 ? "text-emerald" : "text-ruby"}`}>{netTotal >= 0 ? "+" : ""}£{Math.abs(netTotal).toFixed(2)}</strong></span>
          </div>
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="w-full sm:max-w-md">
                <SheetHeader>
                  ...
                  <SheetDescription className="text-xs sm:text-sm">Refine transaction list for this account</SheetDescription>
                </SheetHeader>
                <div className="mt-4 sm:mt-6 space-y-4 sm:space-y-5">
                  <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    <select value={filters.tx_type} onChange={(e) => toggleFilter("tx_type", e.target.value)}
                    ...
                    <select value={filters.source} onChange={(e) => toggleFilter("source", e.target.value)}
                      className="h-10 px-4 rounded-xl bg-secondary/50 border border-transparent text-sm focus:border-ring focus:ring-2 focus:ring-ring/30 focus:outline-none transition-colors">
          ...
          {txLoading ? (
            ...
            <div className="p-6">
              <EmptyState icon={Receipt}
                title={filters.search || filters.category ? "No matching transactions" : "No transactions yet"}
                description={filters.search || filters.category ? "Try adjusting your search or filters." : "Transactions from this account will appear here after syncing."}
              />
            </div>
          ) : (
            <>
