    ...
    <div className="space-y-8" data-testid="reports-root">
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <span className="ml-auto text-2xl sm:text-3xl tracking-tight font-light">
            {data.health_score}
        ...
        <div className="mt-3 sm:mt-4 flex flex-wrap items-center gap-x-4 sm:gap-x-6 gap-y-1.5 sm:gap-y-2 text-xs sm:text-sm">
          ...
          <span className="flex items-center gap-2">
            ...
            {topSpend ? (
              <>
                <CategoryBadge category={topSpend.name} size="sm" />{" "}
                <span className="font-medium">
                  £{topSpend.value.toFixed(0)}
                </span>
              </>
            ) : (
          ...
          <span>
            Subscriptions:{" "}
            <span className="font-medium">
              {subs ? `£${subs.value.toFixed(2)}/mo` : "None detected"}
            </span>
          </span>
          <span>
            Cash flow:{" "}
      ...
      <SectionCard eyebrow="Savings" title="AI savings suggestions">
        <ul className="mt-4 space-y-3 text-sm">
          <li className="flex gap-3">
            <span className="text-emerald">→</span> Cancel unused subscriptions
            could save{" "}
            <span className="text-emerald font-medium">
              £{(subs?.value || 0).toFixed(2)}
            </span>{" "}
            / mo.
          </li>
          <li className="flex gap-3">
            <span className="text-emerald">→</span> Aim to keep{" "}
            {topSpend ? (
            ...
            below 30% of monthly spend.
          </li>
          <li className="flex gap-3">
            <span className="text-emerald">→</span> Set aside 10% of net income
            for Maaser before discretionary spend.
          </li>
          <li className="flex gap-3">
            <span className="text-emerald">→</span> Build an emergency fund of
            3-6 months expenses before increasing investments.
          </li>
        </ul>
      </SectionCard>
          ...
          <div className="mt-5 space-y-5">
            {d.month_grade && (
              <div className="flex items-center gap-4">
                <div
                  className={`text-6xl tracking-tight font-light ${GRADE_COLOURS[d.month_grade] || "text-foreground"}`}
                >
                  {d.month_grade}
                </div>
                <div className="flex-1">
                  ...
                  {d.metrics && (
                    <p className="text-xs text-muted-foreground mt-1">
                      Income £{d.metrics.income?.toFixed(2)} · Spend £
                      {d.metrics.spend?.toFixed(2)} · Saved{" "}
                      {d.metrics.savings_rate_pct}%
                    </p>
                  )}
                </div>
              </div>
