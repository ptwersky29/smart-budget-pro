...
import { Trash2, Plus, Sparkles, Building2, ShieldCheck } from "lucide-react";
...
export default function Settings() {
  ...
  const saveTl = async (e) => {
    ...
    try {
      ...
      if (!payload.client_secret) delete payload.client_secret; // don't overwrite if blank
  ...
  useEffect(() => { load(); refresh(); loadTl(); }, [load, refresh, loadTl]);
  ...
  return (
    <div className="space-y-8" data-testid="settings-root">
      ...
      {user?.role === "admin" && (
        <div className="rounded-2xl border border-border bg-card p-6" data-testid="truelayer-admin-card">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            Configure your TrueLayer sandbox or live app. Register the redirect URI below in the TrueLayer Console.
          ...
          <form onSubmit={saveTl} className="grid sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">Add this exact URL in TrueLayer Console → App → Redirect URIs.</p>
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              ...
              <a href="https://console.truelayer.com" target="_blank" rel="noreferrer" className="ml-3 text-sm text-emerald hover:underline">Open TrueLayer Console ↗</a>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        <p className="label-overline">AI usage</p>        <div className="grid sm:grid-cols-2 gap-3 mt-3">
            ...
            providers.map(p => (
              <div key={p.provider_id} className="flex items-center justify-between p-3 rounded-xl bg-secondary/40">
                <div className="flex items-center gap-3"><Sparkles className="h-4 w-4 text-emerald" />
                  <div><p className="text-sm font-medium">{p.name} {p.is_default && <span className="text-xs text-emerald">· default</span>}</p>
                  <p className="text-xs text-muted-foreground">{p.provider} · {p.model}</p></div>
                </div>
