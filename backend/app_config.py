"""Admin-managed app configuration (TrueLayer credentials, etc.) stored in MongoDB.
...
async def get_config(db, key: str, env_fallback: Optional[str] = None) -> Optional[str]:
...
async def get_truelayer_config(db, user_id: Optional[str] = None) -> dict:
    # Per-user override takes priority over admin/global
    per_user = {}
    if user_id:
        u = await db.users.find_one({"user_id": user_id}, {"_id": 0, "truelayer": 1})
        per_user = (u or {}).get("truelayer") or {}
    cid = per_user.get("client_id") or await get_config(db, "truelayer_client_id", "TRUELAYER_CLIENT_ID")
    sec = per_user.get("client_secret") or await get_config(db, "truelayer_client_secret", "TRUELAYER_CLIENT_SECRET")
    redir = per_user.get("redirect_uri") or await get_config(db, "truelayer_redirect_uri", "TRUELAYER_REDIRECT_URI")
    env = per_user.get("environment") or await get_config(db, "truelayer_environment", None) or "sandbox"
    if not redir:
...
def build_router() -> APIRouter:
    ...
    async def get_tl_config(request: Request, user: dict = Depends(get_current_user)):
        ...
        cfg = await get_truelayer_config(request.app.state.db)
        return {
