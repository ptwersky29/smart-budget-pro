...
async def create_tables():
    ...
    if _engine:
        async with _engine.begin() as conn:
            ...
            await conn.execute(text("ALTER TABLE budgets ADD COLUMN IF NOT EXISTS event_group_id VARCHAR(64)"))
            await conn.execute(text("ALTER TABLE budgets ADD COLUMN IF NOT EXISTS event_group_name VARCHAR(255)"))
            await conn.execute(text("CREATE INDEX IF NOT EXISTS idx_budgets_event_group ON budgets(event_group_id)"))
