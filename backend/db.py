...
async def create_tables():
    ...
    if _engine:
        async with _engine.begin() as conn:
            ...
            await conn.execute(
                text("ALTER TABLE users ADD COLUMN IF NOT EXISTS picture VARCHAR(512)")
            )
            await conn.execute(
                text(
            ...
            await conn.execute(
                text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS free_trial_end TIMESTAMPTZ"
                )
            )
            await conn.execute(
                text(
            ...
            await conn.execute(
                text(
                    "ALTER TABLE transactions ADD COLUMN IF NOT EXISTS normalized_merchant VARCHAR(255)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE transactions ADD COLUMN IF NOT EXISTS parent_id VARCHAR(64)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE transactions ADD COLUMN IF NOT EXISTS recurring_id VARCHAR(64)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE transactions ADD COLUMN IF NOT EXISTS subscription_name VARCHAR(255)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE sms_messages ADD COLUMN IF NOT EXISTS sender_phone VARCHAR(32)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE sms_messages ADD COLUMN IF NOT EXISTS dedup_hash VARCHAR(64)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS data_exported_at TIMESTAMPTZ"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS data_deleted_at TIMESTAMPTZ"
                )
            )
            await conn.execute(
                text(
            ...
            await conn.execute(
                text(
                    "ALTER TABLE budgets ADD COLUMN IF NOT EXISTS budget_type VARCHAR(16) DEFAULT 'everyday'"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE budgets ADD COLUMN IF NOT EXISTS event_date TIMESTAMPTZ"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE budgets ADD COLUMN IF NOT EXISTS event_group_id VARCHAR(64)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE budgets ADD COLUMN IF NOT EXISTS event_group_name VARCHAR(255)"
                )
            )
            await conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_budgets_event_group ON budgets(event_group_id)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS app_language VARCHAR(8) DEFAULT 'en'"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS app_theme VARCHAR(16) DEFAULT 'system'"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS app_currency VARCHAR(4) DEFAULT 'GBP'"
                )
            )
            await conn.execute(
                text("ALTER TABLE truelayer_states ADD COLUMN IF NOT EXISTS meta JSON")
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS last_sync_at TIMESTAMPTZ"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS last_sync_status VARCHAR(32)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS last_error TEXT"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS last_error_at TIMESTAMPTZ"
                )
            )
            # Float → Numeric for monetary columns
            await conn.execute(
                text(
                    "ALTER TABLE transactions ALTER COLUMN amount TYPE NUMERIC(12,2) USING amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE split_transactions ALTER COLUMN amount TYPE NUMERIC(12,2) USING amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE budgets ALTER COLUMN amount TYPE NUMERIC(12,2) USING amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE recurring_transactions ALTER COLUMN amount TYPE NUMERIC(12,2) USING amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE maaser_ledger ALTER COLUMN income_amount TYPE NUMERIC(12,2) USING income_amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE maaser_ledger ALTER COLUMN maaser_due TYPE NUMERIC(12,2) USING maaser_due::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE maaser_ledger ALTER COLUMN maaser_paid TYPE NUMERIC(12,2) USING maaser_paid::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE payment_transactions ALTER COLUMN amount TYPE NUMERIC(12,2) USING amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE billing_records ALTER COLUMN amount TYPE NUMERIC(12,2) USING amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE statements ALTER COLUMN total_income TYPE NUMERIC(12,2) USING total_income::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE statements ALTER COLUMN total_expenses TYPE NUMERIC(12,2) USING total_expenses::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE statements ALTER COLUMN net_savings TYPE NUMERIC(12,2) USING net_savings::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE investment_holdings ALTER COLUMN shares TYPE NUMERIC(14,4) USING shares::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE investment_holdings ALTER COLUMN cost_basis TYPE NUMERIC(14,4) USING cost_basis::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE investment_holdings ALTER COLUMN current_price TYPE NUMERIC(14,4) USING current_price::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE investment_holdings ALTER COLUMN target_allocation_pct TYPE NUMERIC(8,4) USING target_allocation_pct::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE market_data ALTER COLUMN price TYPE NUMERIC(14,4) USING price::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE market_data ALTER COLUMN previous_close TYPE NUMERIC(14,4) USING previous_close::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE market_data ALTER COLUMN change_pct TYPE NUMERIC(8,4) USING change_pct::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE market_data ALTER COLUMN high_52w TYPE NUMERIC(14,4) USING high_52w::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE market_data ALTER COLUMN low_52w TYPE NUMERIC(14,4) USING low_52w::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE holiday_budgets ALTER COLUMN budgeted_amount TYPE NUMERIC(12,2) USING budgeted_amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE holiday_budgets ALTER COLUMN actual_amount TYPE NUMERIC(12,2) USING actual_amount::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE chasuna_plans ALTER COLUMN estimated_cost TYPE NUMERIC(12,2) USING estimated_cost::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE chasuna_plans ALTER COLUMN actual_cost TYPE NUMERIC(12,2) USING actual_cost::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE chasuna_plans ALTER COLUMN deposit_paid TYPE NUMERIC(12,2) USING deposit_paid::numeric"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE analytics_events ALTER COLUMN value TYPE NUMERIC(12,2) USING value::numeric"
                )
            )
            # Currency case fix
            await conn.execute(
                text(
                    "ALTER TABLE payment_transactions ALTER COLUMN currency SET DEFAULT 'GBP'"
                )
            )
            await conn.execute(
                text(
                    "UPDATE payment_transactions SET currency = 'GBP' WHERE currency = 'gbp'"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE billing_records ALTER COLUMN currency SET DEFAULT 'GBP'"
                )
            )
            await conn.execute(
                text(
                    "UPDATE billing_records SET currency = 'GBP' WHERE currency = 'usd'"
                )
            )
            ...
            await conn.execute(
                text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS onboarding_step VARCHAR(64)"
                )
            )
            await conn.execute(
                text("ALTER TABLE users ADD COLUMN IF NOT EXISTS preferences JSON")
            )
            await conn.execute(
                text(
                    "ALTER TABLE users ADD COLUMN IF NOT EXISTS app_language VARCHAR(8) DEFAULT 'en'"
            ...
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS import_start_date DATE"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS update_count INTEGER DEFAULT 0"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS config JSON"
                )
            )
            # Phase 3 — Nicknames, balances
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS nickname VARCHAR(255)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS balance NUMERIC(14,2)"
                )
            )
            await conn.execute(
                text(
                    "ALTER TABLE bank_connections ADD COLUMN IF NOT EXISTS balance_currency VARCHAR(3) DEFAULT 'GBP'"
                )
            )
            await conn.execute(
                text(
            ...
            # Category metadata upgrades
            await conn.execute(
                text(
                    "ALTER TABLE categories ADD COLUMN IF NOT EXISTS label VARCHAR(255)"
                )
            )
            await conn.execute(
            ...
            await conn.execute(
                text(
                    "ALTER TABLE categories ADD COLUMN IF NOT EXISTS section VARCHAR(128)"
                )
            )
            await conn.execute(
                text(
            ...
            # Per-month budgets
            await conn.execute(
                text("ALTER TABLE budgets ADD COLUMN IF NOT EXISTS month VARCHAR(7)")
            )
            await conn.execute(
                text(
            ...
            await conn.execute(
                text(
                    "UPDATE budgets SET month = TO_CHAR(NOW() AT TIME ZONE 'UTC', 'YYYY-MM') WHERE budget_type = 'everyday' AND month IS NULL"
                )
            )
            ...
            await conn.execute(
                text("DROP INDEX IF EXISTS uq_budgets_everyday_user_cat")
            )
            await conn.execute(
                text(
                    "CREATE UNIQUE INDEX IF NOT EXISTS uq_budgets_everyday_user_cat_month ON budgets(user_id, category, month) WHERE budget_type = 'everyday'"
                )
            )
            # Performance indexes
            await conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_transactions_user_date_desc ON transactions(user_id, date DESC)"
                )
            )
            await conn.execute(
                text(
            ...
            await conn.execute(
                text(
                    "CREATE INDEX IF NOT EXISTS idx_maaser_ledger_tx ON maaser_ledger(transaction_id)"
                )
            )
            await conn.execute(
                text(
...
class User(Base, TimestampMixin):
    ...
    free_trial_end: Mapped[Optional[datetime]] = mapped_column(
    ...
    data_exported_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    data_deleted_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
...
class SyncLog(Base):
    ...
    __table_args__ = (Index("idx_sync_logs_user_created", "user_id", "created_at"),)
...
class Budget(Base, TimestampMixin):
    ...
    budget_type: Mapped[str] = mapped_column(
        String(16), default="everyday"
    )  # "everyday" | "event"
    event_date: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    event_group_id: Mapped[Optional[str]] = mapped_column(
        String(64), nullable=True, index=True
    )
    ...
    __table_args__ = (
        ...
        Index(
            "uq_budgets_everyday_user_cat_month",
            "user_id",
            "category",
            "month",
            unique=True,
...
class MaaserLedger(Base, TimestampMixin):
    ...
    income_amount: Mapped[float] = mapped_column(
        Numeric(12, 2, asdecimal=False), default=0
    )
    maaser_due: Mapped[float] = mapped_column(
        Numeric(12, 2, asdecimal=False), default=0
    )
    maaser_paid: Mapped[float] = mapped_column(
        Numeric(12, 2, asdecimal=False), default=0
    )
    ...
    __table_args__ = (Index("idx_maaser_user_date", "user_id", "date"),)
...
class PaymentTransaction(Base, TimestampMixin):
    ...
    ipg_transaction_id: Mapped[Optional[str]] = mapped_column(
        String(128), nullable=True
    )
    signature_valid: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True)
    raw_response: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    notify_received_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
...
class InvestmentHolding(Base, TimestampMixin):
    ...
    cost_basis: Mapped[float] = mapped_column(
        Numeric(14, 4, asdecimal=False), default=0
    )
    ...
    notes: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    price_updated: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    target_allocation_pct: Mapped[float] = mapped_column(
        Numeric(8, 4, asdecimal=False), default=0
    )

    __table_args__ = (Index("idx_investment_user_ticker", "user_id", "ticker"),)
...
class MarketData(Base, TimestampMixin):
    ...
    previous_close: Mapped[float] = mapped_column(
        Numeric(14, 4, asdecimal=False), default=0
    )
...
class SupportTicket(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_support_user_status", "user_id", "status"),)
...
class BudgetOccasionCategory(Base, TimestampMixin):
    ...
    occasion_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("budget_occasions.id"), nullable=False, index=True
    )
    name: Mapped[str] = mapped_column(String(128), nullable=False)
    budgeted_amount: Mapped[float] = mapped_column(
        Numeric(12, 2, asdecimal=False), default=0
